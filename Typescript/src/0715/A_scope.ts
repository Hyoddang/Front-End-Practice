...
function scopeExaple() {
    // 함수 실행 시 작동할 동장: 코드 블록
    var abc = 123;
    if(true) {
        var abc = 456;
    }
...
//! 호이스팅
// : 변수의 선언과 초기화가 동시에 이루어졌을 때, 자바스크립트 인터프리터가 변수의 선을 함수 맨 위로 이동시키는 동작
...
let goOut = true;
if (true) {
    let goOut = false; // let을 이용할 경우 []대괄호 블록과 if블록 내에서의 선언은 블록 안에서만 의미O
}
