...
let employee1: Employee = new Employee('Lee', 1); // 인스턴스 생성
...
//? 독립성
// : 각 인스턴스는 서로 독립적!
// : 한 인스턴스의 상태가 변경 되더라도 다른 인스턴스에는 영향을 주지 않는다.

let employee2: Employee = new Employee('Park', 2);
...
let manager1: Manager = new Manager('Choi', 2, 'HR');
manager1.showDepartment(); // HR
manager1.description(); // Choi의 직원 번호는 2입니다.
...
// 클래스는 설계도에 불과하므로, 실제 작업을 수행하려면 이 설계도를 바탕으로 실제 객체를 생성한다.
// 이 때 생성된 객체가 인스턴스
