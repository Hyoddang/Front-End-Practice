...
class Book {
    ...
    //* !연산자 작성 전
    //* 이니셜라이저가 없고 생성자에 할당되어 있지 않습니다.ts(2546) 오류 발생

    //* ! : ts 컴파일러에게 '값이 무조건 할당 되어있다' 를 명시적으로 알려주는 키워드
}
...
// : 필수적인 요소는 X, 생성자가 없는 경우 TS는 빈 생성자를 자동으로 추가
...
// 생성자의 함수 시그니처와 맞지 않는 타입의 인스턴스화를 시도할 시 에러 발생
...

{
    class Car{
        ...
        constructor(name: string, year: number) {
            // this를 사용해 현재 인스턴스의 name과 year멤버 변수에 접근
            this.name = name;
            this.year = year;
        }
