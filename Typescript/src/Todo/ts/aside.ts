class AsideEvent {
    // 싱글톤 패턴
    // : 특정 클래스의 instance가 하나만 생성되도록 제한하는 패턴
    ...
    public addEventShowMenuButton(): void {
        // '.menu-button' 클래스를 가진 HTML요소 선택 시 해당 요소를 변수에 할당
        const menuButton: HTMLFormElement | null = document.querySelector('.menu-button');

        // menuButton이 존재할 경우, 클릭 이벤트를 추가
        if (menuButton) {
            menuButton?.addEventListener('click', () => {
                // '.menu-aside'클래스를 가진 HTML요소를 선택해서 'menuAside'에 할당
    ...
    public addEventMainChange(): void {
        ...
        menuItems.forEach((menuItem, index) => {
            ...
            menuItem.onclick = () => {
                ...
                // '.menu-aside' 클래스를 가진 HTML요소를 menuAside에 할당
