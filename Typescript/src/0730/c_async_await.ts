...
//     await 키워드 사용(해당 Promise가 resolve될 때까지 기다림)
...
async function hello2() {
    ...
    const result = await promise; // 프로미스가 resolve될 떄까지 기다림.
    console.log(result);
}
...
async function logFirstRepo(url: string) {
    ...
    try {
        ...
        console.log('First repo : ', repos[1]);
    } catch(error) {
        console.log('An Error occureed: ', error)
    }
}
