...
// catch(): 거부(실패)상태의 Promise를 처리하는데 사용
    ...
    console.log(error);
// finally(): Promise의 이행, 거부 여부와 상관업시 호출
    ...
    return new Promise<void>((resolve, reject) => {
        ...
        setTimeout(function() {
            ...
            if (destiny > 0.5) {
                ...
                // 무작위 수가 0.5 보다 작은 경우: 프로미스가 '거부'상태가 되고 reject()를 호출
                reject();
            }
        })
    })
...
fetchDataP()
    .then((result) => {
        ...
        // 처리하고 남음 데이터를 result2로 전달
    })
    .then((result2) => {
        // result2를 가지고 다음 작업 처리
        // 마지막 잡업일 경우 .catch()를 사용하여 에러처리
    })
    .catch((error) => {
        // 에러 처리
        console.log(error);
...
function main() {
    downloadFile('file1.txt')
        .then(() => downloadFile('file2.tst'))
        .then(() => downloadFile('file3.tst'))
        .then(() => console.log('모든 파일이 다운로드 완료 되었습니다.'))
...
//! Promise.race()
// : 여러 개의 Promise 중 가장 먼저 결과를 반환하는 Promise의 결과만 반환하는 새로운 Promise를 생성
