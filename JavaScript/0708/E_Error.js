...
//? 런타임오류(RuntimeError)
// 코드 실행 중에 발생하는 오류
// 존재하지 않는 함수를 호출 || 정의되지 않은 변수를 참조 || 숫자를 0으로 나누려고 할 때 런타임 오류 발생
...
// try블록: 에러가 발생할 수 있는 코드를 포함, 이 코드가 정상적으로 실행되면 catch 블록은 건너뛰고, finally블록이 실행

// catch블록: try블록에서 에러가 발생하면, 실행이 중단되고 제어는 해당 에러 정보와 함께 catch블록으로 이동

// finally블록: try와 catch블록의 실행 후에 항상 실행
...
function exampleFunction () {
    ...
    try {
        ...
        console.log('This message will not be display');
    } catch (error) {  
